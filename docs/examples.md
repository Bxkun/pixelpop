...
class ImageBrowser {
  ...
  async displayCurrentImage() {
    ...
    console.clear();
    console.log(`📷 Image ${this.currentIndex + 1}/${this.images.length}: ${fileName}`);
  ...
  async run(directory: string) {
    ...
    while (true) {
      ...
      switch (command) {
        ...
        default:
          console.log('❓ Unknown command. Use N(ext), P(revious), G(if), or Q(uit)');
      }
    }
  }
...
class TestVisualizer {
  ...
  private async displayTestResult(result: TestResult) {
    ...
    console.log(`${statusIcon} ${chalk[statusColor](result.name)} (${result.duration}ms)`);
...
const results: TestResult[] = [
  ...
  { name: 'API endpoint validation', status: 'fail', duration: 205, error: 'Expected 200, got 500' },
...
class AsciiArtGenerator {
  ...
  async generateFromImage(imagePath: string, options: {
    width?: number;
    height?: number;
    colored?: boolean;
    inverted?: boolean;
    ...
    try {
      ...
      for (let y = 0; y < image.bitmap.height; y++) {
        ...
        for (let x = 0; x < image.bitmap.width; x++) {
          ...
          // Map to character
          const charIndex = Math.floor(adjustedBrightness * (this.chars.length - 1));
  ...
  async displayCurrentArtwork() {
    ...
    try {
      if (artwork.isAnimated) {
        console.log(chalk.yellow('🎬 Animated artwork - press any key to stop'));
...
class VisualSystemMonitor {
  ...
  private async updateDisplay() {
    console.clear();
    console.log(chalk.blue('📊 System Status') + ' - ' + new Date().toLocaleTimeString());
  ...
  class BatchImageProcessor {
    ...
    async processDirectory(directoryPath: string) {
      try {
        ...
        console.log(`🔄 Processing ${imageFiles.length} images with ${this.numWorkers} workers`);
    ...
    private async processChunk(files: string[], workerIndex: number): Promise<number> {
